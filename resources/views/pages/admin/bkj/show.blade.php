...
<div class="container-fluid px-4 py-4">
    <div class="row">
        <!-- Menggunakan col-12 agar layout melebar penuh dan memanfaatkan ruang layar admin secara maksimal -->
        <div class="col-12">
            
            <!-- HEADER ACTION BAR (Gaya Melayang Minimalis) -->
            <div class="d-flex flex-sm-row flex-column align-items-sm-center justify-content-between mb-4 pb-3 border-bottom gap-3">
                <div>
                    <h2 class="h4 mb-1 fw-bold text-dark lh-sm">Daftar Badan Kelengkapan Jurusan</h2>
                    <p class="text-muted small mb-0">Manajemen data Badan Kelengkapan di Jurusan Promosi Kesehatan.</p>
                </div>
                <div>
                    <a href="/badan_kelengkapan_jurusan/create" class="btn btn-primary rounded-3 px-3 py-2 text-white shadow-xs d-inline-flex align-items-center gap-2 small fw-bold">
                        <i class="bi bi-plus-lg fs-6"></i> &nbsp; Tambah Data BKJ
                    </a>
                </div>
            </div>

            <!-- STATUS NOTIFIKASI / ALERTS -->
            @if (session('pesan'))
            ...
            <div class="card border-0 shadow-sm rounded-4 overflow-hidden" style="border-top: 4px solid #0d6efd !important;">
                ...
                <div class="card-header bg-white border-bottom py-3.5 px-4 d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
                    <div class="d-flex align-items-center gap-2 text-dark">
                        <h6 class="mb-0 fw-bold">Data Komponen BKJ</h6>
                    </div>
                    
                    <div class="d-flex align-items-center gap-2">
                        @if (Request()->keyword != '')
                            <a href="/badan_kelengkapan_jurusan" class="btn btn-light border btn-sm rounded-3 d-flex align-items-center px-3 text-secondary" style="height: 36px; font-size: 0.85rem;">
                        ...
                        <form class="input-group input-group-sm m-0" style="max-width: 320px;" method="GET" action="/badan_kelengkapan_jurusan">
                            <input type="text" 
                                   ...
                                   name="keyword" placeholder="Cari Badan Kelengkapan..." value="{{ Request()->keyword }}"
                    ...
                    <table class="table table-hover align-middle mb-0 text-nowrap custom-table-hover">
                        <thead class="table-light text-secondary text-uppercase fs-7 small border-bottom fw-bold">
                            <tr>
                                ...
                                <th scope="col" class="py-3.5 text-start" style="width: 30%">Nama Badan Kelengkapan</th>
                        ...
                        <tbody class="fs-6 border-top-0">
                            ...
                            <tr>
                                ...
                                <td class="text-center py-3">
                                    @if($data->logo)
                                        <div class="d-inline-block p-1 bg-light rounded-3 border shadow-xs">
                                            <img src="{{ asset('uploads/bkj/' . $data->logo) }}" alt="Logo {{ $data->nama_bkj }}" class="rounded-2" style="height: 42px; width: 42px; object-fit: cover;">
                                        </div>
                                    @else
                                ...
                                <td>
                                    <div class="fw-bold text-dark fs-6">{{ $data->nama_bkj }}</div>
                                    <div class="text-muted small opacity-75 mt-0.5">ID Komponen: #BKJ-{{ $data->id_bkj }}</div>
                                </td>
                                <td>
                                    @if($data->medsos && $data->medsos != '-')
                                        <span class="badge bg-primary-subtle text-primary border border-primary-subtle px-3 py-2 rounded-pill fw-semibold small d-inline-flex align-items-center gap-1.5">
                                            {{ $data->medsos }}
                                        </span>
                                    @else
                                ...
                                <td class="text-center pe-4">
                                    <div class="d-inline-flex gap-1.5">
                                        <a href="/badan_kelengkapan_jurusan/{{ $data->id_bkj }}" class="btn btn-sm btn-light border rounded-3 text-primary custom-action-btn shadow-xs" title="Lihat Detail Profil">
                                            <i class="bi bi-eye-fill"></i>
                                        </a>
                                        <a href="/badan_kelengkapan_jurusan/{{ $data->id_bkj }}/edit" class="btn btn-sm btn-light border rounded-3 text-warning custom-action-btn shadow-xs" title="Ubah Konfigurasi">
                                            <i class="bi bi-pencil-fill"></i>
                                        </a>
                                        <button type="button" class="btn btn-sm btn-light border rounded-3 text-danger custom-action-btn shadow-xs" data-bs-toggle="modal" data-bs-target="#hapus{{ $data->id_bkj }}" title="Hapus Permanen">
                            ...
                            <tr>
                                <td colspan="5" class="text-center py-5 text-muted">
                                    <div class="py-4">
                                        ...
                                        <p class="text-muted small mb-0 px-3">Kata kunci pencarian Anda tidak cocok dengan record entitas BKJ mana pun di sistem.</p>
                                    </div>
                                </td>
                            </tr>
                            @endforelse
...
<div class="modal fade" id="hapus{{ $data->id_bkj }}" tabindex="-1" aria-labelledby="modalLabel{{ $data->id_bkj }}" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" style="max-width: 420px;">
        <form action="/badan_kelengkapan_jurusan/{{ $data->id_bkj }}" method="POST" class="modal-content border-0 shadow rounded-4 overflow-hidden">
            @csrf
            @method('DELETE')
            ...
            <div class="modal-body text-center p-4">
                ...
                <h5 class="fw-bold text-dark mb-2" id="modalLabel{{ $data->id_bkj }}">Konfirmasi Hapus Data</h5>
                <p class="text-muted small mb-4 px-2">Tindakan ini bersifat ireversibel. Apakah Anda yakin ingin menghapus data komponen ormawa/bkj ini dari database?</p>
                
                <div class="p-3 bg-light rounded-3 fw-bold text-primary border text-center small mb-2 border-dashed">
                    <i class="bi bi-layers me-1.5"></i> {{ $data->nama_bkj }}
                </div>
            </div>
