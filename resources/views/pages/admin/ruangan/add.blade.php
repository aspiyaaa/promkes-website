...
<div class="container-fluid px-4 py-5" style="background-color: #f8fafc; min-height: 100vh;">
    ...
    <div class="d-flex flex-column flex-sm-row align-items-sm-center justify-content-between mb-5 pb-4 border-bottom gap-3">
        <div>
            <h2 class="h3 mb-2 fw-bold text-dark d-flex align-items-center gap-2">
                <i class="bi bi-plus-circle-fill text-primary"></i> Tambah Data Ruangan
            </h2>
            <p class="text-muted small mb-0">Kelola inventaris ruangan, tipe, kapasitas, lokasi, beserta kelengkapan fasilitasnya.</p>
        </div>
        <div>
    ...
    @if ($errors->any())
        <div class="alert alert-danger alert-dismissible fade show rounded-4 mb-4 p-4 shadow-sm" role="alert">
            <h6 class="fw-bold mb-2 text-danger d-flex align-items-center gap-2">
                <i class="bi bi-exclamation-triangle-fill fs-5"></i> Gagal Menyimpan Data!
            </h6>
    ...
    <form action="/ruangan" method="POST" enctype="multipart/form-data">
        @csrf
                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="row g-4">
                            <div class="col-12">
                                ...
                                <input type="text" class="form-control form-custom rounded-3 @error('nama_ruangan') is-invalid @enderror" id="nama_ruangan" name="nama_ruangan" value="{{ old('nama_ruangan') }}" placeholder="Contoh: Ruang Kelas Teori 2A" required>
                                @error('nama_ruangan')
                            ...
                            <div class="col-12">
                                ...
                                <input type="text" class="form-control form-custom rounded-3 @error('kode_ruangan') is-invalid @enderror" id="kode_ruangan" name="kode_ruangan" value="{{ old('kode_ruangan') }}" placeholder="Contoh: R-PROMKES-01" required>
                                @error('kode_ruangan')
                                    <div class="invalid-feedback mt-2">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-12">
                                ...
                                <select class="form-select form-custom rounded-3 @error('tipe_ruangan') is-invalid @enderror" id="tipe_ruangan" name="tipe_ruangan" required style="min-height: 46px;">
                                    <option value="" disabled {{ old('tipe_ruangan') == '' ? 'selected' : '' }}>-- Pilih Tipe Ruangan --</option>
                                    <option value="kelas" {{ old('tipe_ruangan') == 'kelas' ? 'selected' : '' }}>Kelas</option>
                                    <option value="laboratorium" {{ old('tipe_ruangan') == 'laboratorium' ? 'selected' : '' }}>Laboratorium</option>
                                    <option value="aula" {{ old('tipe_ruangan') == 'aula' ? 'selected' : '' }}>Aula</option>
                                </select>
                                @error('tipe_ruangan')
                                    <div class="invalid-feedback mt-2">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-12">
                                ...
                                <input type="text" class="form-control form-custom rounded-3 @error('lokasi') is-invalid @enderror" id="lokasi" name="lokasi" value="{{ old('lokasi') }}" placeholder="Contoh: Gedung Direktorat Lantai 2" required>
                                @error('lokasi')
                                    <div class="invalid-feedback mt-2">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-12">
                                ...
                                <div class="input-group">
                                    <input type="number" class="form-control form-custom rounded-start-3 @error('kapasitas') is-invalid @enderror" id="kapasitas" name="kapasitas" value="{{ old('kapasitas') }}" placeholder="40" min="1" required style="border-right: none;">
                ...
                <div class="card border-0 shadow-sm rounded-4 bg-white p-3">
                    <div class="card-body p-4">
                        ...
                        <div class="mb-5 p-4 bg-light rounded-4 border-start border-success border-4 shadow-xs">
                            ...
                            <div class="d-flex flex-column flex-sm-row gap-3 mt-3">
                                <div class="form-check custom-radio-wrapper flex-fill">
                                    <input class="form-check-input d-none" type="radio" name="status" id="status_tersedia" value="tersedia" {{ old('status', 'tersedia') == 'tersedia' ? 'checked' : '' }}>
                                ...
                                <div class="form-check custom-radio-wrapper flex-fill mb-0">
                                    <input class="form-check-input d-none" type="radio" name="status" id="status_digunakan" value="digunakan" {{ old('status') == 'digunakan' ? 'checked' : '' }}>
                                ...
                                <div class="form-check custom-radio-wrapper flex-fill mb-0">
                                    <input class="form-check-input d-none" type="radio" name="status" id="status_perbaikan" value="perbaikan" {{ old('status') == 'perbaikan' ? 'checked' : '' }}>
                        ...
                        <div class="mb-5 p-4 bg-light rounded-4 border-start border-info border-4">
                            ...
                            <textarea class="form-control rounded-3 bg-white border-light-subtle p-3 @error('fasilitas') is-invalid @enderror" id="fasilitas" name="fasilitas" rows="3" placeholder="Contoh: AC 2 Unit, Proyektor LCD, Smart TV, Whiteboard" required style="font-size: 0.9rem; line-height: 1.6; border-color: #cbd5e1;">{{ old('fasilitas') }}</textarea>
                            @error('fasilitas') <div class="invalid-feedback mt-2">{{ $message }}</div> @enderror
                        </div>
                        ...
                        <div class="mb-5 p-4 bg-light rounded-4 border-start border-primary border-4">
                            ...
                            <textarea class="form-control rounded-3 bg-white border-light-subtle p-3 @error('deskripsi') is-invalid @enderror" id="deskripsi" name="deskripsi" rows="2" placeholder="Masukkan deskripsi singkat fungsi atau kondisi khusus ruangan..." style="font-size: 0.9rem; line-height: 1.6; border-color: #cbd5e1;">{{ old('deskripsi') }}</textarea>
                        ...
                        <div class="mb-3 p-4 bg-light rounded-4 border-start border-warning border-4">
                            ...
                            <div class="form-text text-muted small mt-2 d-flex align-items-center gap-1">
                                <i class="bi bi-info-circle"></i> Format: JPG, JPEG, PNG, WEBP. Maksimal ukuran berkas 2MB.
                            </div>
        ...
        <div class="d-flex justify-content-end align-items-center gap-3 mt-5 pt-4 border-top">
            <button type="reset" class="btn btn-light rounded-3 px-4 py-2 border small text-secondary fw-semibold">Reset Form</button>
            <button type="submit" class="btn btn-success rounded-3 px-5 py-2 fw-bold shadow-sm border-0" style="background-color: #10b981; min-height: 44px;">
                <i class="bi bi-save me-1"></i> Simpan Data Ruangan
            </button>
        </div>
    </form>
</div>
...
<style>
    ...
    /* Custom Radio Buttons Style (Anti Mepet) */
